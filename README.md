...
| PreToolUse hook | `$CLAUDE_CONFIG_DIR/settings.json` | .env, 키파일, lock, .git 편집 차단 (protect-files) |
| PreCompact hook | `$CLAUDE_CONFIG_DIR/settings.json` | 컨텍스트 압축 전 트랜스크립트 백업 (backup-transcript) |
| 시스템 프롬프트 패치 | cli.js 직접 수정 | 62개 패치, 매 API 호출마다 ~10.7K 토큰 절감 |
...
node patch-cli.js  # 62/62 패치, ~10.7K 토큰 절감
```
...
| [CLA/README.md](./CLA/README.md) | 핵심 모듈 상세 (10 스킬, 6 스크립트, 7 템플릿) |
